  ...
  return (
    <div className="space-y-5">
      <header className="rounded-2xl border border-cac-line bg-white p-5 shadow-cac">
        ...
        <h1 className="mt-1 text-2xl font-black text-cac-navy">{t('admin.repTitle')}</h1>
        <p className="mt-2 max-w-3xl text-sm text-cac-muted">{t('admin.repSupport')}</p>
        <p className="mt-4 inline-flex rounded-full border border-cac-line bg-[#fbfcfb] px-3 py-1.5 text-xs font-black text-cac-navy">
          ...
          {items.map((item) => (
            <li key={item.id} className="rounded-2xl border border-cac-line bg-white p-4 shadow-cac">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-base font-black text-cac-navy">
                ...
                <span className="mt-2 inline-flex w-fit rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-amber-900 sm:mt-0">
                  {t('admin.repStatusRequested')}
                </span>
              </div>
              {item.interest ? (
                <p className="mt-3 rounded-xl border border-cac-line bg-[#fbfcfb] px-3 py-2 text-sm text-cac-navy">
                  {item.interest}
                </p>
              ) : null}
