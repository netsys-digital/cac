      ...
      <header className="overflow-hidden rounded-2xl border border-cac-line bg-white shadow-cac">
        <div className="grid gap-0 lg:grid-cols-[1.4fr_1fr]">
          <div className="p-5 xl:p-6">
            ...
            <h1 className="mt-2 text-grande font-bold leading-tight text-cac-navy">
              {t('curator.title', { name: firstName })}
            </h1>
            <p className="mt-2 max-w-2xl text-media leading-relaxed text-cac-muted">
              {t('curator.subtitle')}
            </p>
            <p className="mt-3 inline-flex items-center gap-2 rounded-full border border-cac-line bg-[#fbfcfb] px-3 py-1.5 text-media font-bold text-cac-navy">
          ...
          <div className="border-t border-cac-line bg-[#fbfcfb] p-5 lg:border-t-0 lg:border-l xl:p-6">
            ...
            <ul className="mt-3 space-y-2 text-media text-cac-navy">
              <li className="flex gap-2">
                <span className="text-cac-green font-bold">1.</span>
                {t('curator.role1')}
              </li>
              <li className="flex gap-2">
                <span className="text-cac-green font-bold">2.</span>
                {t('curator.role2')}
              </li>
              <li className="flex gap-2">
                <span className="text-cac-green font-bold">3.</span>
                {t('curator.role3')}
              </li>
              <li className="flex gap-2">
                <span className="text-cac-green font-bold">4.</span>
                {t('curator.role4')}
              </li>
            </ul>
          </div>
        ...
        <div className="flex flex-col rounded-2xl border border-cac-line bg-white p-5 shadow-cac">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-1 text-grande font-bold text-cac-navy">{t('admin.curateTitle')}</h2>
              <p className="mt-1 text-media text-cac-muted">{t('curator.queueContentHint')}</p>
            </div>
            <span
              className={`grid min-w-10 place-items-center rounded-full px-2.5 py-1 text-media font-bold ${
          ...
          <ul className="mt-4 flex-1 space-y-2">
            {pendingContent.slice(0, 4).map((item) => (
              ...
              >
                ...
                <p className="mt-0.5 text-media font-bold text-cac-navy">{item.title}</p>
          ...
          <Link
            to="/admin/curate"
            className="mt-4 inline-flex items-center justify-center rounded-[12px] bg-cac-navy px-4 py-3 text-media font-extrabold text-white transition hover:bg-cac-green2"
          >
        ...
        <div className="flex flex-col rounded-2xl border border-cac-line bg-white p-5 shadow-cac">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-1 text-grande font-bold text-cac-navy">{t('admin.repTitle')}</h2>
        ...
        <div className="rounded-2xl border border-cac-line bg-white p-5 shadow-cac">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-6">
            {KPI_KEYS.map((key) => (
              ...
              >
                <b className="block text-grande font-bold text-cac-navy">{kpis?.[key] ?? '—'}</b>
